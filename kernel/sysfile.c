...
uint64 sys_mmap(void) {
	...
	for (int i = 0; i < VMA_NUM; i++) {
		...
		} else if (vaend > p->vmas[i].vastart) {
...
uint64 sys_munmap(void) {
	...
	struct vma *v = 0;
	for (int i = 0; i < VMA_NUM; i++) {
		if (p->vmas[i].valid == 1 && p->vmas[i].vastart <= va && va < p->vmas[i].vastart + p->vmas[i].sz) {
			v = &p->vmas[i];
		}
	}
	if (v == 0) return -1;
	...
	uint64 addr_aligned = va;
	if (va > v->vastart) {
		addr_aligned = PGROUNDUP(va);
	}

	int nunmap = sz - (addr_aligned - va); // nbytes to unmap
	if (nunmap < 0)
		nunmap = 0;

	vmaunmap(p->pagetable, addr_aligned, nunmap, v); // custom memory page unmap routine for mmapped pages.

	if (va <= v->vastart && va + sz > v->vastart) { // unmap at the beginning
...
int can_lazy_alloc(uint64 va) {
	...
	if (v == 0) return 0;
	void *pa = kalloc();
	if (pa == 0) { panic("can_lazy_alloc: kalloc fail"); }
	memset(pa, 0, PGSIZE);
	begin_op();
	ilock(v->f->ip);
	...
	if (mappages(p->pagetable, va, PGSIZE, (uint64)pa, PTE_R | PTE_W | PTE_U) < 0) {
		panic("can_lazy_alloc: mappages fail");
		return 0;
	}
